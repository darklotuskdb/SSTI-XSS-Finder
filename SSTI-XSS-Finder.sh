...
if [ -z "$1" ]
	...
	for TECH in $w_tech;
		do
			...
			for tar in $b; 
				do 
					echo -e "\n\n\e[1;34m[*]$tar\e[0m => \e[1;31mAngularJS $(wappalyzer https://$tar -P | sed -n -e '/angularjs/,/website/ p' | grep version | tr -d '"version": ",' | tee v.txt) \e[0m \n\n\e[1;33m[+]XSS Payloads\n\e[0m $(g=$(cat v.txt | awk '{ print substr( $0, 1, length($0)-2 ) }') && grep "$g" Payloads.txt)";
				done
